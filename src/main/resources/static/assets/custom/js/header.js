...
document.write("<img src='assets/images/users/face.jpg' alt='user' class='rounded-circle'> <span class='ml-1 pro-user-name'>" +loggedInUser + " <i class='mdi mdi-chevron-down'></i> </span>");
document.write("</a>");
...
document.write("</a>");
document.write("<a href='javascript:void(0)' onclick='logout()' class='dropdown-item notify-item border-0'>");
...
function logout() {
    var csrfToken = $("meta[name='_csrf']").attr("content"); // CSRF token
    var csrfHeader = $("meta[name='_csrf_header']").attr("content"); // CSRF header name

    $.ajax({
        type: "POST",
        url: "/logout",
        beforeSend: function(xhr) {
            xhr.setRequestHeader(csrfHeader, csrfToken); // Attach CSRF token
        },
